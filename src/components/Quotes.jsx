...
export default function Quotes() {
  ...
  return (
    <div className="bg-zinc-900 h-screen flex justify-center items-center">
      {/* display Quotes */}
      <div className="">
        <div className="border rounded-2xl px-12 py-5 h-[222px]  flex justify-center items-center">
      ...
      <div
        ...
        onClick={toggleMenu}
      >
        {onmenu ? <X  size={30} className="bg-red-400"/> : <Menu  size={30}/>}
      </div>
      <nav
        className={`fixed h-screen top-0 left-0 transition-opacity duration-500 ease-in-out z-10 ${
          onmenu ? "opacity-100 visible " : "opacity-0 invisible "
        }`}
      >
        <ul
          className={` text-[2em] text-[#7a7c7c]  bg-zinc-800 h-screen  p-5 fixed top-0 left-0 transition-transform duration-500 ease-in-out transform ${
